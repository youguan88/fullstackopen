...
describe("api with token", () => {
  beforeEach(async () => {
    ...
    await Promise.all(
      helper.users.map(async (user) => await api.post("/api/users").send(user)),
    );

    const login = await api
      .post("/api/login")
      .send(helper.loginBody(helper.users[0]));
  ...
  test("delete is valid", async () => {
    ...
    expect(newReturnedBlogs.body.map((x) => x.id)).not.toContain(
      addedBlog.body.id,
    );
  });
  ...
  test("update is valid", async () => {
    ...
    const newReturnedBlogs = await api.get("/api/blogs");
    const updatedBlog = newReturnedBlogs.body.filter(
      (blog) => blog.id === addedBlog.body.id,
    );
